...
def save_csv_hm(sample,score,hm_dir_basis,hm_sorting,csv_path,th,area_th,blendingfactor):
    ...
    img=img_transposetorch2nparr(image.cpu().numpy())
    ...
    #print(normalized_hm_colored.shape)
    ...
    #axarr[0][1].imshow(cv2.addWeighted(img_transposetorch2nparr(image.cpu().numpy()),blendfactor,cv2.cvtColor(score, cv2.COLOR_GRAY2BGR),(1-blendfactor),1))
...
def generate_result_path(trainer):
    ...
    hm_dir_basis=trainer.save_path+'/heatmaps/'+trainer.param_str+"/"
    if not os.path.isdir(hm_dir_basis):
        os.mkdir(hm_dir_basis)
    hm_dir_basis=hm_dir_basis+test_timestamp+"/"
    if not os.path.isdir(hm_dir_basis):
        os.mkdir(hm_dir_basis)
    return hm_dir_basis,csv_path
